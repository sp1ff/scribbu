...
BOOST_AUTO_TEST_CASE( test_com )
{
  ...
  comments comm = C.data();
  BOOST_CHECK( 0 == comm.unicode() );

  unsigned char buf[3];
  BOOST_CHECK( buf + 3 == comm.lang(buf) );
  BOOST_CHECK( 'e' == buf[0] && 'n' == buf[1] && 'g' == buf[2] );

  BOOST_CHECK( buf + 3 == comm.description(buf) );
  BOOST_CHECK( 'f' == buf[0] && 'o' == buf[1] && 'o' == buf[2] );

  BOOST_CHECK( buf + 3 == comm.text(buf) );
  BOOST_CHECK( 'b' == buf[0] && 'a' == buf[1] && 'r' == buf[2] );

} // End test_com.

// TODO: Unit test class CNT
// TODO: Unit test class POP
